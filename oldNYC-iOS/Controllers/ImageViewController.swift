...
final class ImageViewController: UIViewController, UIScrollViewDelegate, UIGestureRecognizerDelegate, UIViewControllerTransitioningDelegate {
    ...
    fileprivate let maximumZoomScale: CGFloat = 4
    ...
    func updateImageAndContentSize(_ image: UIImage) {
        ...
        if let handler = fadeInHandler, handler.wasPresented(self.index) == false {
            
            if self.index != self.startIndex {
                
                activityIndicatorView.stopAnimating()
                
                UIView.transition(with: self.scrollView, duration: 0.5, options: UIView.AnimationOptions.transitionCrossDissolve, animations: { [weak self] () -> Void in
