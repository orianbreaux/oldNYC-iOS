...
public final class ImageViewer: UIViewController, UIScrollViewDelegate, UIViewControllerTransitioningDelegate {
    ...
    public var detailButtonActionInitiationBlock: (() -> Void)?
    /// Executed as the last step for detail button's close action.
    public var detailButtonActionCompletionBlock: (() -> Void)?
    /// Executed as the first step before the button's detail action starts.
    public var shareButtonActionInitiationBlock: (() -> Void)?
    /// Executed as the last step for detail button's close action.
    public var shareButtonActionCompletionBlock: (() -> Void)?
    ...
    fileprivate func configureCloseButton() {
        ...
        closeButton.setImage(closeButtonAssets.normal, for: UIControl.State())
    ...
    fileprivate func configureDetailButton() {
        ...
        detailButton.setImage(detailButtonAssets.normal, for: UIControl.State())
    ...
    fileprivate func configureShareButton() {
        ...
        shareButton.setImage(shareButtonAssets.normal, for: UIControl.State())
    ...
    public override func loadView() {
        ...
        overlayView.autoresizingMask = UIView.AutoresizingMask()
    ...
    @objc func detail(_ sender: AnyObject) {
        ...
        let OKAction = UIAlertAction(title: "View Item in NYPL Collection", style: .default) { UIAlertAction in
            ...
            if let url = URL(string: "http://www.jogendra.com") {
            ...
            let doneItem = UIBarButtonItem(barButtonSystemItem: UIBarButtonItem.SystemItem.done, target: nil, action: #selector(GalleryViewController.removeWebView));
    ...
    func showAnimation(_ duration: TimeInterval, completion: ((Bool) -> Void)?) {
        ...
        UIView.animate(withDuration: duration, animations: {
            ...
            if finished {
                if isPortraitOnly() {
                    NotificationCenter.default.addObserver(self, selector: #selector(ImageViewer.rotate), name: UIDevice.orientationDidChangeNotification, object: nil)
                }
    ...
    override public func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey: Any]?, context: UnsafeMutableRawPointer?) {
        ...
        if (dynamicTransparencyActive == true && keyPath == "contentOffset") {
            ...
            detailButton.alpha = 1 - abs(scrollView.contentOffset.y / distanceToEdge) * transparencyMultiplier
            ...
            shareButton.alpha = 1 - abs(scrollView.contentOffset.y / distanceToEdge) * transparencyMultiplier
