...
class MapViewController: UIViewController,
    ...
    @IBAction func tappedCenterOnUserbutton(_ sender: UIButton) {
        ...
        let toCamera = MGLMapCamera(lookingAtCenter: (mapView.userLocation?.coordinate)!, altitude: fromCamera.altitude, pitch: 0, heading: 0)

        mapView.setCamera(toCamera, withDuration: 0.5, animationTimingFunction: CAMediaTimingFunction(name: CAMediaTimingFunctionName.linear), completionHandler: {() -> Void in self.mapView.setUserTrackingMode(.follow, animated:false)})
    }
...
//********** FUNCTIONS FOR GENERATING MAP UI **********//
        ...
        if let path = Bundle.main.path(forResource: "markers", ofType: "json") {
            do {
                ...
                if jsonObj != JSON.null {
                    ...
                    markersLayer.circleColor = NSExpression(forConstantValue:  #colorLiteral(red: 1, green: 0, blue: 0, alpha: 1))
                    let stops = [
                        12: 3,
                        22: 180
                    ]
                    markersLayer.circleRadius = NSExpression(format: "mgl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'exponential', 1.75, %@)", stops)
                    markersLayer.circleOpacity = NSExpression(forConstantValue: 0.6)
    ...
    func centerOnUserLocation(_ currentCoordinates: CLLocationCoordinate2D) {
        ...
        let toCamera = MGLMapCamera(lookingAtCenter: currentCoordinates, altitude: fromCamera.altitude, pitch: 0, heading: 0)
        
        self.mapView.setCamera(toCamera, withDuration: 0.75, animationTimingFunction: CAMediaTimingFunction(name: CAMediaTimingFunctionName.linear), completionHandler: {() -> Void in
